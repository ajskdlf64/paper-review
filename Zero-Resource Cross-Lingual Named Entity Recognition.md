...
- **Cross-Word**: We project source and target word embeddings to a common space using the unsupervised mapper ($W_{s->t} or W_{t->s}$). This model uses word-level crosslingual information learned from adversarial training and
the Procrustes-CSLS refinement procedure.
- **Cross-Shared**: This model is the same as Cross-Word, but the weights of the forward and backward LSTM cells are shared to encourage order invariance in the model. - **Cross-Augmented**: This is our full cross-lingual model trained with source labels and target pseudo-labels generated by the pretrained model and the model itself.
### Results
